...
<div style="max-width: 600px; margin: 0 auto;">
    <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 2rem;">
        <div>
            <h2 style="font-size: 1.5rem; font-weight: 700;">Chỉnh sửa bàn: {{ $table->name }}</h2>
        </div>
    ...
    <div class="card">
        <form action="{{ route('tables.update', $table->id) }}" method="POST">
            ...
            <div style="margin-bottom: 1.5rem;">
                ...
                <input type="text" name="name" value="{{ old('name', $table->name) }}" required style="width: 100%; padding: 0.75rem; border-radius: 0.5rem; border: 1px solid #e2e8f0;">
            ...
            <div style="margin-bottom: 1.5rem;">
                ...
                <select name="status" required style="width: 100%; padding: 0.75rem; border-radius: 0.5rem; border: 1px solid #e2e8f0; background: white;">
                    ...
                    <option value="occupied" {{ old('status', $table->status) == 'occupied' ? 'selected' : '' }}>Có khách</option>
                </select>
            </div>

            <div style="margin-bottom: 2rem;">
                <label style="display: block; margin-bottom: 0.5rem; font-weight: 500;">Mô tả/Vị trí</label>
                <textarea name="description" rows="3" style="width: 100%; padding: 0.75rem; border-radius: 0.5rem; border: 1px solid #e2e8f0;">{{ old('description', $table->description) }}</textarea>
            </div>

            <div style="display: flex; gap: 1rem; justify-content: flex-end;">
                <button type="submit" class="btn btn-primary" style="padding: 0.75rem 2rem;">
                    <i class="fas fa-save"></i> Cập nhật bàn
                </button>
            </div>
        </form>
    </div>
